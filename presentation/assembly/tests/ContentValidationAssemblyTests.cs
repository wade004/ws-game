...
namespace Tests.Presentation.Assembly
{
    ...
    public class ContentValidationAssemblyTests
    {
        ...
        /// 判断记录），方法名同步改掉已不再成立的"两项"描述。**再次扩为五项（ADR-0038 落地）**：新增
        /// <c>RefCategoryFieldRule</c>，同样默认关闭（见 <see cref="ContentValidationOptions.EnableRefCategoryCheck"/>
        /// 判断记录——样例数据尚未迁移到 ADR-0038 新前缀，开启会拖垮 <c>--strict</c> 门禁），方法名不再
        /// 改（"四项"已是历史命名，同 <c>OptionalRuleNames</c> 本身命名惯例不随条目数变化）。</summary>
        [Fact]
        ...
        /// 行为变更（ADR-0038 落地）**：新增 <c>RefCategoryFieldRule</c>，同样默认关闭（见
        /// <see cref="ContentValidationOptions.EnableRefCategoryCheck"/> 判断记录），计入
        /// <c>DisabledOptionalRules</c> 末尾。</summary>
        [Fact]
        public void Run_EnabledDisabledOptionalRuleChecks_CorrespondToRuleNameLists()
        {
            ...
            Assert.Equal(
                new[] { "QuestPrerequisiteIsolationRule", "TalentTreeIsolationRule", "RefCategoryFieldRule" },
                run.DisabledOptionalRules);
            Assert.Equal(
                new[] { QuestPrerequisiteIsolationRule.CheckName, TalentTreeIsolationRule.CheckName, RefCategoryFieldRule.CheckName },
        ...
        /// 那两条可选规则的默认启用行为。**ADR-0038 落地补充**：新增的 <c>RefCategoryFieldRule</c>
        /// 同样默认关闭，一并计入 <c>DisabledOptionalRules</c>。</summary>
        [Fact]
        ...
        public void Run_WithBothOptionalRuleDependencies_BothEnabled_AndDisplayMapCoverageRuleActuallyFires()
        {
            ...
            // 规则按默认值关闭，计入 DisabledOptionalRules——不是"仍恒为空"。ADR-0038 落地：本用例
            // 也未设置 EnableRefCategoryCheck，RefCategoryFieldRule 同样按默认值关闭。
            Assert.Equal(
                new[] { "QuestPrerequisiteIsolationRule", "TalentTreeIsolationRule", "RefCategoryFieldRule" },
                run.DisabledOptionalRules);
            Assert.Equal(
