...
- **运行时解码贴图带多级渐远链**（消费方反馈第四十二批，阻塞，
  [ADR-0096](architecture/adr/0096-运行时解码贴图带多级渐远链.md)）：地图分层图/精灵集静态图像/
  逐帧动画三条运行时解码路径默认开启 mip 链与三线性过滤（可关，关闭时自动降级为双线性，与改动前
  逐字节一致），根治源资产密度高于屏幕显示密度时的欠采样锯齿与移动闪烁；逐帧动画开启后按帧切成
  独立纹理避免同一图集内相邻帧渗色，显存开销上升约三分之一。
- **回避类结算发布事件**（消费方第四十三批反馈2，阻塞，
  [ADR-0098](architecture/adr/0098-回避类结算发布事件.md)）：新增事件 `combat.attack_avoided`
  （`CombatAttackAvoidedEvent`），未命中/闪避/招架（结算判定终止分支）与免疫吸收判定为免疫时
  统一发布；`HitResult` 枚举新增 `Immune` 成员。不改动 `combat.damage_dealt`/`combat.heal_done`
  的既有发布条件与字段。
...
- **逐帧动画枢轴与像素密度取自所属精灵集**（消费方反馈第四十批，阻塞，
  [ADR-0095](architecture/adr/0095-逐帧动画枢轴与像素密度取自所属精灵集.md)）：属于精灵集的
  逐帧动画（`sprite_anim.*`）此前枢轴固定几何中心、像素密度固定全局默认值，现随加载请求携带所属
  精灵集提示，按该集 `anchors.json` 脚底锚点换算，与同集静态图像表现一致；特效序列帧（`vfx.*`）
  不受影响。修正 ADR-0091 决策 5 对逐帧动画"共用解码路径"的错误表述。

## [1.76.0] - 2026-09-26

### 新增

- **默认动画剪辑随朝向档位切换**（消费方反馈第三十九批，阻塞，
  [ADR-0093](architecture/adr/0093-动画剪辑随朝向档位切换.md)）：默认挂接的逐层/整身剪辑此前只在
  实体挂接那一刻按当时朝向探测一次，运行期换向不会重新探测；现方向槽位变化时自动重新探测（表现层
  新增受保护虚方法钩子，ABI 加法），换向后保留同一状态内已播放的时长，整身路线补齐与逐层同款的
  方向探测能力。
- **朝向约定新增 `MirrorFacingY`/`FacingAngleOffsetRadians` 两个口味项，与方向档位数无关**
  （消费方反馈第三十九批，阻塞，
  [ADR-0094](architecture/adr/0094-朝向约定口味项与档位数无关.md)）：在量化之前对原始朝向角先镜像
  再偏移，默认值下逐字节不变；既有的按索引重映射配置项长度与档位数不一致时，由纯静默改为一次性
  Warn 诊断（未接诊断出口时仍纯静默），处理结果本身不变。

## [1.75.0] - 2026-09-26

### 新增

- **玩家主动放弃任务**（消费方反馈第三十八批，阻塞，
  [ADR-0092](architecture/adr/0092-玩家主动放弃任务意图.md)）：新增 `IQuestHost.Abandon`（默认
  接口成员）与 `UiIntents.AbandonQuest` 两重载、事件 `quest.abandoned`；放弃回到"上一次交付之后
  的状态"（从未交付过等价于回到未接取，已交付过则保留交付历史、回落到上一次交付刚完成时的形态），
  均不产生失败记录，与既有 `Fail` 语义区分。

## [1.74.0] - 2026-09-26

### 修复

- **精灵枢轴取自精灵集脚底锚点，行为变更**（消费方反馈第三十七批，阻塞，
  [ADR-0091](architecture/adr/0091-精灵枢轴取自精灵集脚底锚点.md)）：精灵集图像/逐帧动画的枢轴
  此前固定为几何中心，现改为按该精灵集 `anchors.json` 声明的脚底锚点 `root` 换算；未声明/解析
  失败/越界回退几何中心，逐字节不变。**行为变更**：全部已声明 `root` 的精灵集渲染位置会整体上移
  （对齐脚底而非图中心）。
...
- **区域事件驱动的循环音效"离开后停不掉"排查结论**（消费方反馈第三十六批）：框架链路无缺陷——`area.trigger_*`
  对每个单位各发一次，未按单位过滤的 `play_sfx{attach: source}` 会给区域内每个单位（含静止 NPC）各起一个
  实例，玩家离开只停自己那个；数据侧在两条规则条件上追加单位过滤即可。新增生产装配级复现/对照用例
  `AreaTriggerLoopSfxProductionChainTests`，[ADR-0089](architecture/adr/0089-循环音效与stop_sfx动作.md)"已知限制"节追加。
...
- **AI 战斗态新增回追转移 `combat_to_chase`，`chase_to_combat` 默认阈值同步收紧**（消费方反馈
  第二十九批，阻塞项，
  [ADR-0084](architecture/adr/0084-AI战斗态目标脱离攻击范围回追.md)）：`core/rules/ai`
  `AiHost` 的 `combat` 态此前只刷新目标、判定逃跑/脱战两个既有转移，从不产生位移意图，也没有
  任何路径能在"目标仍存活、仍在追击范围内、但已超出攻击距离"时把单位带出 `combat`——此前**此
  类单位会永久卡在原地反复施法失败**（含开启"战斗中参战"的召唤物，联动既有"战斗中不跟随主人"
  策略后全程冻结）。**行为变更（一）**：此前会冻结的单位，现在目标脱离攻击距离时会自动回追
  （复用 `chase` 态既有的移动与拴绳判定），追出拴绳范围仍按既有逻辑脱战返回，不会无限追击；
  `combat_to_chase` 固定用裸 `AttackRange`，不加任何余量，`combat` 期间不存在死区。**行为变更
  （二）**：滞回余量改放在"重新进战" `chase_to_combat` 一侧——`AiOptions` 新增
  `CombatReentryRangeRatio`（取值域 `(0, 1]`，默认 0.75，越界抛异常不静默夹紧），
  `chase_to_combat` 默认判定由裸 `AttackRange` 收紧为 `AttackRange * CombatReentryRangeRatio`，
  单位需要追得比以前更近才会重新开打，避免目标停在攻击距离边界附近时来回翻转；已用 Expr 覆盖
  `chase_to_combat` 的内容数据不受影响。两条新/改转移均可被 `ai.behavior_profile.transitions`
  的 Expr 覆盖（覆盖键 `combat_to_chase`/`chase_to_combat`），与既有转移同等地位。数值仿真三份
  基线（`sim_arena_matrix`/`sim_coverage_all`/`sim_growth_full`）重新比对后仍为零差异（未触发
...
- **创建期异常隔离与诊断信息不丢失**（消费方反馈第三十二批阻塞项，ADR-0085 同批复核遗留，
  [ADR-0086](architecture/adr/0086-创建期异常隔离与诊断信息不丢失.md)）：
  - `Presentation.ViewBinding.ViewBinder.OnEntityCreated` 建 View 失败（视图工厂抛异常）时不再
    向上传播——捕获异常、不写入绑定表、记一条带实体 id/kind/displayId/异常类型/异常消息/堆栈的
    诊断，正常返回；`OnSaveLoaded` 内部遍历多个实体的三段循环（补建、销毁陈旧 View、装备外观
    对账）同样逐条隔离。**根治此前"补建循环里任意一个实体建 View 抛异常，会中断整个循环，导致
    排在它后面的全部实体都拿不到 View"这一放大效应**——这与消费方所报"跨图读档后全部存活生物
    无 View"的现象形态吻合。
  - `Core.Foundation.EventBus.EventBus` 捕获订阅者异常时，诊断消息文本新增订阅者标识（"声明
    类型.方法名"）与异常类型全名/消息（原先只有事件 key 一项，排查不出是谁抛的、抛的什么）；
    完整异常对象（含堆栈）仍按原有参数传递。
  - 引擎适配层诊断转发不再丢失异常信息：事件总线/钩子注册表/应用生命周期/存档系统/场景路由五处
    "消息 + 可选异常"诊断源转发到宿主控制台时带上异常类型全名与堆栈，去重键额外纳入异常类型
    全名与堆栈最顶一帧（不再只按消息文字），重复出现不再静默丢弃——转发一条带累计次数的精简
    提示。诊断契约本身只做加法，不改既有成员签名。
  - `Core.Gameplay.Assembly.GameplayAssembly.AttachSceneRouter` 新增运行时同实例校验：能判断
    传入场景路由构造时使用的钩子注册表时，与本类型的钩子注册表不是同一实例就立即抛异常（此前
    这一装配错误会让跨图读档的 `save.loaded` 补发一直挂起，且没有任何异常/诊断）。
...
- **并行门禁判定用例放宽阈值，消除满载误判**（无对外契约变化）：`toolchain/tests/test_gate_step_runner.py`
  的"两条线确实并行"判定此前判定带只有 5s（并行上限 35s、串行下限 40s），而该用例自身嵌套起一个
  门禁子进程的固定开销实测已达 16.6s、超出原本 15s 的预算，满载时必然落进带内误判（1.67.0 发布
  门禁实测 36.59s 翻红，该值仍明显低于串行下限，两条线确实是并行的）。固定开销预算放宽到 25s，
  单段模拟耗时加倍到 40s——固定开销不随它增长，判定带因此由 5s 拉开到 15s（65~80s）。

- **消费方演练的残留 Unity 进程判定漏掉逐参数加引号的命令行**（无对外契约变化）：
  `toolchain/_unity_smoke_wait_scope_guard.ps1` 解析 `-projectPath` 归属时，正则要求开关名后面
  紧跟空白；而按参数列表启动进程时每个参数（含开关名自身）都被单独加引号，开关名后面紧跟的是
  引号，于是整条命令行被判成"没有 `-projectPath`"→ `Unknown` → 按保守口径等待。后果是同一台
  机器上别的仓库里正常运行的 Unity 批处理也会被等满 60s 超时并判失败——这类进程不是"刚退出还在
  清理"，等多久都不会消失（1.67.0 发布门禁实测：`toolchain/consumer_smoke.ps1` 因此连挂 3 步）。
  开关名后放宽为允许一个可选闭合引号；`toolchain/tests/test_unity_smoke_wait_scope_guard.py`
  新增 3 例钉死这种写法下的三态判定（修复前 3 例全红、均返回 `Unknown`）。

- **ADR-0074 落地缺陷：additive 占位材质在经独立分发通道安装的消费方工程里等同于没生效**（消费方
  反馈第二十四批）：引擎适配层原按包内相对路径检索占位材质的机制，在"以源码/内嵌形态直接引用"下
  正常，但在"经独立分发通道按版本号安装的不可变只读包"这一消费形态下会检索不到（已用一个从未见过
  本仓库源码的独立工程实测复现并排除打包遗漏），每次都触发"降级为默认（alpha）材质"的兜底，观感
  与完全没接 additive 分支一致。改为运行期按着色器名称检索后现场构造材质，不再依赖按路径检索包内
  专用目录资产这条在该消费形态下有限制的机制；同时新增编辑器侧自动登记，保证该着色器不会被独立
  构建产物的未使用资源裁剪掉（消费方不需要任何手工步骤）。`blend_mode` 不声明或声明 `alpha` 时的
  材质选择路径未改动。`toolchain/consumer_smoke.ps1` 新增一个覆盖"经独立分发通道按版本号安装"这一
  此前从未被本脚本走过的消费路径的步骤，防同类缺陷复发。详见
  [ADR-0074](architecture/adr/0074-粒子发射混合模式.md)"落地缺陷与修复（2026-09-23）"小节。
- **`feedback.binding` 同一事件命中多条规则时的异常隔离**（消费方第二十五批反馈排查引出，
  [ADR-0077](architecture/adr/0077-ui交互域事件.md)"落地缺陷与修复"一节、
  `presentation/feedback_binder/README.md` 判断记录 18）：`FeedbackBinder.OnEvent` 处理同一个
  事件命中的多条规则时，一条规则的 `condition` 求值抛异常或某个 `action` 派发抛异常，此前会
  中止本次调用剩余的全部处理（同一事件命中的其它规则、同一规则内后续动作都被误跳过）。现在
  `condition` 求值与每条 `action` 派发分别独立隔离，抛出时只记一条诊断、跳过它自己，不影响
  同一事件的其它规则/动作，也不影响后续任何事件；不改变既有立即派发语义。
  **消费方报告的两个具体现象本身仍未复现，本版未修复它们**：一次 `ui.panel_opened` 影响后续不
  相关事件的绑定、`ui.action_invoked -> play_sfx` 确认已派发但未观测到播放——两条先后经核心层 +
  表现层（确定性用例）与真实引擎适配层运行期（真实事件总线、真实装配根、真实资源加载与音频播放，
  且贴合消费方原配置：`ui.panel_opened` 自带一条指向冷资源的 `play_sfx` 绑定、冷加载进行中紧接着
  发出 `item.equipped`）两轮复现，均未能重现；消费方原始运行日志亦为零异常、零错误、相关组件名
  定向检索零命中。已排除项清单、消费方与框架侧环境差异清单、消费方可自行执行的诊断步骤，见
  ADR-0077"落地缺陷与修复（2026-09-23）"及其"续"小节。
...
- **销毁时序对齐与待销毁单位跳过处理**（消费方反馈第二十二批，阻塞项，
  [ADR-0079](architecture/adr/0079-销毁时序对齐与待销毁单位跳过处理.md)）：`ICreatureFactory.Despawn`
  此前同步注销属性/能量宿主的注册，而世界模拟对该实体的真正移除要等到某次模拟步的生命周期清理
  阶段——两次调用之间若该实体仍有生效的 AI 移动意图，下一拍移动系统会向已注销的属性宿主要属性，
  抛异常中止整拍，实体没被真正移除，下一拍同样路径再抛一次，永久卡死主循环。根治：`Despawn` 不再
  同步注销，属性/能量宿主的注销改为响应"实体已销毁"事件、与世界真正移除同一批生效；`IWorldSim`
  新增只读查询"是否已标记待销毁"（纯加法默认接口成员），AI 决策与移动系统据此在处理前跳过已标记
  销毁的单位。不改变生命周期清理阶段、`CreatureDespawnedEvent` 的既有发出时机与内容。
