...
- **`feedback.binding` 同一事件命中多条规则时的异常隔离**（消费方第二十五批反馈排查引出，
  ...
  同一事件的其它规则/动作，也不影响后续任何事件；不改变既有立即派发语义。消费方报告的两个具体
  现象（一次 `ui.panel_opened` 影响后续不相关事件的绑定；`ui.action_invoked -> play_sfx` 确认
  已派发但未观测到播放）经核心层 + 表现层反复复现均未能重现，判断详情见 ADR-0077 对应小节。
