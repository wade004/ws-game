...
- **AI 战斗态新增回追转移 `combat_to_chase`**（消费方反馈第二十九批，阻塞项，
  ...
  策略后全程冻结）。**行为变更**：此前会冻结的单位，现在目标脱离攻击距离时会自动回追（复用
  `chase` 态既有的移动与拴绳判定），追出拴绳范围仍按既有逻辑脱战返回，不会无限追击；`AiOptions`
  新增 `CombatChaseHysteresis`（默认 1）控制回追判定与"追击进战斗"判定之间的滞回余量，避免目标
  停在攻击距离边界附近时来回翻转。新转移可被 `ai.behavior_profile.transitions` 的 Expr 覆盖
  （覆盖键 `combat_to_chase`），与既有转移同等地位。数值仿真三份基线经比对零差异（此次改动
  未触发任何场景的 `Exceeded`/`Added`/`Removed`），未重烘基线。
