...
- `UnityResourceLoader.cs`：资源 id→路径映射、后台线程 + 主线程完成队列的线程模型、音频/字体
  ...
  **ADR-0095 跟进（2026-09-26，消费方反馈第四十批）勘误**：上一条关于 `TryDecodeEffect` 的表述
  ...
  默认剪辑（`RequestAnimClipUpgrade`）、逐层剪辑（`ProbeLayerClipTier`）与武器风格/技能覆盖剪辑
  （`RequestWeaponClipUpgrade`，经新增的 `_spriteSetIdsByEntity` 按 entityId 反查其所属精灵集——
  与默认状态剪辑同属一批 `sprite_anim` 资源，须走同一枢轴来源，不能只接前两处）三处随加载请求带上
  精灵集提示（前两处取 `display.map.sprite_set_id`，第三处取该实体挂接默认动画时登记的同一个值）；
