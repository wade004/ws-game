...
static class TeleportLoadingBoundaryProbe
{
    ...
    public static void Main()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=The public Dialog OpenGossip/ChooseOption path accepted the second request while the first scene load was still Loading. ApplyResolvedTeleport updated the player to A before swallowing SceneRouter's Loading rejection; completion of the first B load left the router on B but did not correct the player fields, producing a final scene/player divergence.");
    }
}
