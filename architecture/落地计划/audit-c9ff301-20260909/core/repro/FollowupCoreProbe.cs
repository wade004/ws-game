...
static class FollowupCoreProbe
{
    ...
    {
        Console.WriteLine("FOLLOWUP-CORE-PROBE baseline=c9ff301 version=1.13.0");
    ...
    static void RunRollbackClassPowerState()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=The valid A save has the same race, class A, and partially consumed Mana=30 while in combat. Forward B load removes Mana, then failure rollback rebuilds class A resources; current 1.13 PlayerVitals persistence restores Mana=30 and InCombat=True, and Advance(1) leaves both values unchanged under regen_in_combat=0.");
    }
    ...
    static void RunInventoryViewModelAfterLoad()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=UiDataSource queries the live B inventory/equipment after same-map load, and current 1.13 InventoryViewModel refreshes from SaveLoaded; the explicit Refresh below is a repeatability check, not required to make the production load path consistent.");
        inventoryVm.Refresh();
