...
$project = Join-Path $resolvedOutput "repro\SchemaConsumer.csproj"
dotnet build $project -c Release -p:SourceRoot=$source -o $build
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
dotnet (Join-Path $build "SchemaConsumer.dll") *>&1 | Tee-Object -FilePath $log
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
