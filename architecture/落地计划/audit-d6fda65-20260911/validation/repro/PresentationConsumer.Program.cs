...
internal static class Program
{
    private static void Main()
    {
        ...
        WorldStateInt64RoundtripCase();
        WeaponStyleMapKeyCase();
        DisplayOffsetMapKeyCase();
        ExprSchemaKnownKeysCase();
    ...
    private static void WorldStateInt64RoundtripCase()
    {
        const long intended = 9007199254740993L;
        var key = new Id("world.flag.validation_int64");
        var writer = new Id("quest.validation");
        var source = new Core.Gameplay.WorldState.WorldState(MakeBus());
        source.Set(key, ExprValue.OfInt(intended), writer);
        var saved = source.Save();
        var text = JsonWriter.Write(saved);
        var loaded = new Core.Gameplay.WorldState.WorldState(MakeBus());
        loaded.Load(JsonReader.Parse(text));
        var actual = loaded.Get(key);
        Console.WriteLine("WORLDSTATE_INT64 input=" + intended + " expected=exact Save->JsonWriter->JsonReader->Load actual_text=" + text + " actual_kind=" + actual.Kind + " actual_value=" + actual.AsInt + " exact=" + (actual.Kind == ExprValueKind.Int && actual.AsInt == intended));
    }
