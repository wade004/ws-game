...
static class FollowupCoreProbe
{
    ...
    {
        Console.WriteLine("FOLLOWUP-CORE-PROBE baseline=76d16a5 version=1.14.0");
    ...
    static void RunSuccessfulLoadEventSuppression()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=SaveSystem suppression remains active, but current 1.14 DerivedStateRebuilder directly recomputes after the relevant section; rating follows the restored level.");
    }
    ...
    static void RunSuccessfulLoadPowerMaxSuppression()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=The current 1.14 section hook recomputes Power max before PlayerVitals.Load, so saved health is applied against max 200 despite event suppression.");
    }
    ...
    static void RunRollbackDerivedStateAfterFailure()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=Forward load applies B and its rebuild hook, then the missing custom section throws. current 1.14 positive rollback restores A fields and re-runs the derived hook, restoring A stat/aura.");
    }
    ...
    static void RunRollbackVitalsAfterEquipment()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=The B save is valid before fault injection; malformed player.known_skills fails after equipment and before player.vitals. current 1.14 positive rollback restores equipment, re-runs the derived hook, and restores max/health to 200.");
    }
    ...
    static void RunSameMapClassLoad()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=Class B omits A's legacy base-stat key and mana power type. current 1.14 ReloadArchetypeAndRace rebuilds the class contribution and resource set to match B.");
    }
    ...
    static void RunRollbackClassPowerState()
    {
        ...
        var inCombatAfterRollback = powersA.IsInCombat(Unit);
        powersA.Advance(Unit, 1);
        var manaAfterAdvance = powersA.HasPower(Unit, Mana) ? powersA.GetPower(Unit, Mana) : -1;
        var inCombatAfterAdvance = powersA.IsInCombat(Unit);
        Console.WriteLine("ROLLBACK-CLASS-POWER-CURRENT-AND-COMBAT-STATE");
        Console.WriteLine($"save_a_initial={savedAInitial.Success};save_a_with_failure={savedAWithFailure.Success};save_b_valid={savedB.Success};race_before={RaceA.Value};race_after={fxA.Player.RaceId?.Value ?? "<null>"};class_after={fxA.Player.ArchetypeId.Value};mana_before={manaBefore};in_combat_before=True;load_status={loaded.Status};mana_present_after={powersA.HasPower(Unit, Mana)};mana_after_rollback={manaAfterRollback};in_combat_after_rollback={inCombatAfterRollback};mana_after_advance_1={manaAfterAdvance};in_combat_after_advance_1={inCombatAfterAdvance}");
        Console.WriteLine("ORACLE-CURRENT=load_status=PersistableThrew;race_after=arch.race.followup_a;class_after=arch.class.followup_a;mana_present_after=True;mana_after_rollback=30;in_combat_after_rollback=True;mana_after_advance_1=30;in_combat_after_advance_1=True");
        Console.WriteLine("INTERPRETATION-CURRENT=The valid A save has the same race, class A, and partially consumed Mana=30 while in combat. Forward B load removes Mana, then failure rollback rebuilds class A resources; current 1.14 PlayerVitals persistence restores Mana=30 and InCombat=True, and Advance(1) leaves both values unchanged under regen_in_combat=0.");
    }
    ...
    static void RunRollbackClassBeforeRaceFailure()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=The valid B document writes class B before its malformed race section fails. current 1.14 rollback restores the same race and different class in forward order, re-runs derived rebuilding, and preserves the partially consumed Mana and single RaceA aura reference.");
    }
    ...
    static void RunInventoryViewModelAfterLoad()
    {
        ...
        Console.WriteLine("BUG-SIGNATURE-CURRENT=NO-BUG-SIGNATURE;vm_entries_after and vm_equipped_instance_after match the live B direct query after SaveLoaded.");
        Console.WriteLine("INTERPRETATION-CURRENT=UiDataSource queries the live B inventory/equipment after same-map load, and current 1.14 InventoryViewModel refreshes from SaveLoaded; the explicit Refresh below is a repeatability check, not required to make the production load path consistent.");
        inventoryVm.Refresh();
