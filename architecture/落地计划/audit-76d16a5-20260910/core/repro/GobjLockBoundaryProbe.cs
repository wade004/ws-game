...
static class GobjLockBoundaryProbe
{
    ...
    static void Main()
    {
        ...
        catch (Exception ex) { Console.WriteLine($"lockdef_parse=blocked;type={ex.GetType().Name};message={ex.Message}"); }
        ...
        Console.WriteLine("ORACLE-CURRENT=world_flag requirement requires flag_key and expected; missing expected is rejected by formal validation before runtime parsing, while expected=true is valid.");
        Console.WriteLine("INTERPRETATION-CURRENT=1.14 formal validation blocks the missing expected variant, so registry reads are correctly blocked; the positive fixture confirms both formal acceptance and LockDef parsing. Process exit 0 is not semantic proof.");
    }
}
