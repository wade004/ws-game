...
{
    private static WorldSim? LastWorld;
    ...
    {
        Console.WriteLine("BOUNDARY_REPROS HEAD=c86bfa98475dbf348f129c0fa507d117d238914c");
        var mapClear = ReproEquipmentMapClear();
        Console.WriteLine($"SUMMARY ReproEquipmentMapClear={(mapClear ? "REPRODUCED" : "NOT_REPRODUCED")}");
        return mapClear ? 0 : 1;
    }

    private static bool ReproEquipmentMapClear()
    {
        var assembly = BuildDuplicateAssembly(out var player);
        var aura = new Id("skill.aura.repro_shared");
        Equip(assembly, player, new Id("item.repro.ordinary"), new Id("item.slot.repro_ordinary"));
        var beforeAura = assembly.Rules.Skill.AuraQuery.HasAura(player, aura);
        var beforeEquipped = assembly.Equipment.GetEquipped(player, new Id("item.slot.repro_ordinary")) != null;

        LastWorld!.ClearAll();
        assembly.Rules.Bus.DispatchPending();
        var recreated = new Core.Carriers.Unit.PlayerUnit(
            player, new Id("map.repro.b"), new Id("fac.repro"), new Id("arch.repro"));
        LastWorld.AddEntity(recreated);
        assembly.Rules.Bus.DispatchPending();

        var afterAura = assembly.Rules.Skill.AuraQuery.HasAura(player, aura);
        var afterEquipped = assembly.Equipment.GetEquipped(player, new Id("item.slot.repro_ordinary")) != null;
        var reproduced = beforeAura && beforeEquipped && !afterAura && afterEquipped;
        Console.WriteLine("ReproEquipmentMapClear expected=permanent_equipment_and_aura_survive_cross_map_world_clear");
        Console.WriteLine($"ReproEquipmentMapClear actual=beforeAura:{beforeAura} beforeEquipped:{beforeEquipped} afterAura:{afterAura} afterEquipped:{afterEquipped} reproduced:{reproduced}");
        Console.WriteLine("ReproEquipmentMapClear boundary=real CarriersAssembly/AuraHost/EquipmentHost; ClearAll+entity.destroyed removes aura while EquipmentHost ledger remains");
        return reproduced;
    }
    ...
    private static InMemoryDataSource BuildDuplicateSource()
    {
        ...
        source.Add("skill.aura_def", Table("skill.aura_def", "[{\"id\":\"" + aura + "\",\"max_stacks\":1,\"effects\":[{\"kind\":\"mod_stat\",\"params\":{\"stat\":\"stat.power\",\"op\":\"flat\",\"value\":100}}]}]"));
