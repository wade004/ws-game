...
static class FollowupCoreProbe
{
    ...
    {
        Console.WriteLine("FOLLOWUP-CORE-PROBE baseline=ac3b622 version=1.10.0");
    ...
    static void RunSuccessfulLoadEventSuppression()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=SaveSystem suppression remains active, but v1.10 DerivedStateRebuilder directly recomputes after the relevant section; rating follows the restored level.");
    }
    ...
    static void RunSuccessfulLoadPowerMaxSuppression()
    {
        ...
        Console.WriteLine("INTERPRETATION-CURRENT=The v1.10 section hook recomputes Power max before PlayerVitals.Load, so saved health is applied against max 200 despite event suppression.");
    }
    ...
    static void RunDerivedHookExceptionBoundary()
    {
        ...
        Console.WriteLine("EXPECTED=save_success=True;load_status=Loaded;before_calls=1;section_calls>0;warning_count>0");
    ...
    static void RunRollbackDerivedStateAfterFailure()
    {
        ...
        Console.WriteLine("EXPECTED=load_status=PersistableThrew;after_race=arch.race.followup_a;stat=61;auraA=True;auraB=False");
        Console.WriteLine("INTERPRETATION=Forward load applies B and its rebuild hook, then the missing custom section throws. Positive rollback restores A fields but invokes no derived-state hook, leaving B stat/aura in the A live state.");
    }
    ...
    static void RunRollbackVitalsAfterEquipment()
    {
        ...
        Console.WriteLine("EXPECTED=load_status=PersistableThrew;after_level=2;equipped_after=True;max_after=200;health_after=200");
        Console.WriteLine("INTERPRETATION=The B save is valid before fault injection; malformed player.known_skills fails after equipment and before player.vitals. Positive rollback restores equipment but has no derived hook, so max/health remain clamped at the low-equipment state and vitals is never reloaded.");
    }
    ...
    static void RunSameMapClassLoad()
    {
        ...
        Console.WriteLine("EXPECTED=save_a_valid=True;save_b_valid=True;b_oracle_stat=20;b_oracle_legacy=0;b_oracle_mana=False;load_status=Loaded;after_class=arch.class.followup_b;after_stat=20;after_legacy=0;after_mana=False");
        Console.WriteLine("INTERPRETATION=Class B omits A's legacy base-stat key and mana power type. ReloadArchetypeAndRace updates B fields but does not remove an old base key or unregister A's power type.");
    }
    ...
    static void RunWeaponStyleCacheAfterLoad()
    {
        ...
        Console.WriteLine("EXPECTED=load_status=Loaded;style_before=display.weapon_style.followup_a;style_after_before_dispatch=display.weapon_style.followup_a;style_after=display.weapon_style.followup_b;oracle_b_style=display.weapon_style.followup_b");
        Console.WriteLine("INTERPRETATION=Same-map load changes the real EquipmentHost template from A to B, but SaveSystem keeps item events suppressed; after EventBus drain the source cache still returns A instead of the independently primed B source oracle.");
        sourceA.Dispose();
        sourceB.Dispose();
    }
    ...
    static InMemoryDataSource Data()
    {
        ...
        return new InMemoryDataSource()
            ...
            .Add("arch.power_type", E("arch.power_type", "[{\"id\":\"arch.power.health\",\"name_key\":\"l10n.health\",\"max_source\":{\"kind\":\"stat\",\"stat\":\"stat.max_health\"},\"start_full\":true},{\"id\":\"" + Mana.Value + "\",\"name_key\":\"l10n.mana\",\"max_source\":{\"kind\":\"fixed\",\"value\":100},\"start_full\":true}]"))
