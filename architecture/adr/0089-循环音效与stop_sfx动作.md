...
- **缺陷修复（2026-09-26，消费方第三十五批阻塞项）**：本决策首次落地时 `PlayAttached` 只在 `Play`
  同步返回真实句柄时才登记跟踪键，冷资源加载中（`Play` 排队返回 `null`）不登记——真实引擎音频解码
  永远异步，循环音效在进程内的第一次播放必然命中这条路径，`StopAttached`/`stop_sfx` 因此永远查不到
  键，起播成功却停不下来。已改为在排队那一刻登记为"排队中"态、资源加载完成后升级为"已在播"态，
