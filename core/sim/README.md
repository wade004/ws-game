...
54. **`core/sim/README.md`"线程安全与并发"章节与并发回归测试（第 51 条）**：见本文件上方
    ...
    合并时若编号撞车，属预期的文本冲突，整合时按提交先后（本文件先落地 49-51 批次的
    52/53/54 三条）重新顺排为下方 55/56 两条，内容本身无需改动。

## 消费方反馈第 52-53 条判断记录（2026-09-17）

55. **消费方反馈第 53 条——`GrowthSimulation.ResolveCreatureFamily` 按阵营过滤同 `tier` 候选，同
    ...
    无回归。
    <br/>**结论（整合时设计层已拍板，不再是存疑项）**：`ResolveCreatureFamily` 的"同级多候选取
    第一条"延续原有隐式行为，本次落地只补诊断，不改成"报错"或"随机选择"；`OpponentAmbiguities`
    非空**只作警告，不在任何更高层（`check.ps1`/发布流程/CI）升级为硬性阻断**——与
    `SimGrowthOpponentAmbiguityValidationRule`（`NonEscalatable => true`）的既有实现口径一致，
    消费方自己的报告查看器/CI 若需要更严格把关可自行决定，框架不代为决定。`creature.sample_hero`
    这类"玩家阵营占位生物与怪物同 tier 共存"的既有安排，本轮**不系统化**为专门的"占位/不可作战斗
    对手" tier 惯例（不推广 `sample_summon_totem` 本次挪 tier 的处理方式）——按 `data/_sample` 现状
    保留即可，待后续同类反馈频率上升再评估是否值得系统化。

56. **消费方反馈第 52 条——`StandardPlayer.EquippedWeaponSummaries`/`GetEquippedWeaponSummary`
