...
方法。**判断记录（未提升进 `Core.Rules.Common.ISkillHost` 契约）**：本次并行派单明确把
`ISkillHost.cs` 划给另一条并行分支（同批新增 `AuraQuery`/`EffectSink` 只读默认成员），为避免两个
分支同时改同一份契约文件撞车，本次施法查询只停在 `SkillHost` 具体类与 `presentation/ui` 自己的
`ISkillBookQuery` 窄接口（新增 `GetCastingSkillId`/`GetCastingRemaining`/`GetCastingTotal` 三个
默认接口成员，`SkillHostSkillBookQuery` 用向下转型到具体类 `SkillHost` 转发）——同 ADR-0050 之前
`GetKnownSkills` 的同一处境，是已知的临时窄化，留给后续 ADR 一并提升进 `ISkillHost`。
