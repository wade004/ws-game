...
namespace Core.Gameplay.Quest
{
    ...
    /// </para>
    /// <para>
    /// <b>前置引用的提取</b>：复用框架已公开的通用引用遍历入口 <see cref="ExprReferenceCollector"/>
    /// （深度优先收集语法树里全部 <see cref="ExprReferenceNode"/>，含嵌套在参数列表/and/or/not/比较子树
    /// 里的引用，遍历口径与 <c>QuestContentValidationRule.CollectQuestIdLiteralArgs</c> 一致），本方法
    /// 只做"过滤 <c>Group == quest</c> + 取 Id 字面量参数"这一步——不复制/不改动
    /// <c>QuestContentValidationRule</c>（该文件由并行任务改动校验规则本身，本类型不与其冲突），也不需要
    /// <see cref="IExprSchema"/>：直接读取 <see cref="QuestDefinition.Prerequisite"/> 已解析好的
    /// <see cref="ExprNode"/>，不重新解析文本。
    /// </para>
    /// <para>
    ...
    public static class QuestPrerequisitePreview
    {
        ...
        /// 全部 <c>quest.*</c> 引用的 Id 字面量参数，按出现顺序去重。</summary>
        private static IReadOnlyList<Id> ExtractDirectReferences(QuestDefinition def)
        {
            ...
            foreach (var reference in ExprReferenceCollector.Collect(def.Prerequisite))
            {
                ...
                {
                    if (arg is ExprLiteralNode literal && literal.Value.Kind == ExprValueKind.Id)
                    {
                        var id = literal.Value.AsId;
                        if (seen.Add(id))
                        {
                            result.Add(id);
                        }
