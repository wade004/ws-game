namespace Core.Foundation.DataRegistry
{
    ...
    public enum FieldUnit
    {
        ...
        /// <summary>消费方反馈第 61 条（2026-09-18）：角度量，框架内一律弧度制——逆时针为正，
        /// 0 指向 +X 轴（见 05_对象模型与世界.md"单位约定"一节，该节是全仓库角度单位约定的集中声明，
        /// 本枚举成员只是配合登记/导出用的结构化标记）。角度制（度）只允许出现在名字自带
        /// <c>Degrees</c> 的少数接口参数——那些参数不经过本字段登记体系，不受影响。</summary>
        Radian,
    }
}
