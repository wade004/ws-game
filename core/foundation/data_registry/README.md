...
（原名 `消费方问询-2026-09-20-第71条反问答复要求.md`，因问询一直未获答复、设计层拍板不再等待，
已就地改写为通知文档并 `git mv` 重命名）发出后，设计层拍板不再等消费方答复、直接定契约
（不撤销先前的反问，只是不再等待）。落地结论与
...
- **原方案"接入 ADR-0042 DiagnosticsHub"经核实在结构上不成立，未采用**——`DiagnosticsHub`
  ...
  `data_registry` 的 `ValidationReport`/`ValidationIssue` 纳入普查（原问询文档核实过程中确认，
  该核实结论现已整合进 ADR-0046 决策 2）。三点结构性不匹配：① 只能装载字符串，不能装载
