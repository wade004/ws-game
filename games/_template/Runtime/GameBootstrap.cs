...
namespace Game.Template
{
    ...
    public sealed class GameBootstrap : MonoBehaviour
    {
        ...
        /// <c>Adapter.Unity.Presentation.PresentationAssemblyDiagnosticsForwarder</c> 类型注释，同
        /// <c>Adapter.Unity.Bootstrap.GameFoundationBootstrap</c>/<c>Adapter.Unity.Shell.
        ...
        private void Bootstrap()
        {
            ...
            // 整合判断记录（1.45.0）：第 68 条（数据集根覆盖）与第 70 条（内容源物理根覆盖）
            // 是两个正交维度——前者改的是数据集在内容根下的相对子目录，后者改的是内容根本身，
            // 二者可同时生效，也可各自单独使用，互不依赖。两条在各自分支上并行开发时改到了
            // 同一段代码，此处按"都保留、先定数据集根再定内容根"的顺序合并。
