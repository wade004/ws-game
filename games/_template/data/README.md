...
| `stat.definition` | 游戏必填 | 至少玩家职业的主属性一条（`stat/stat.definition.json`）；另需一条 `stat.move_speed`（`MovementOptions.MoveSpeedStat` 默认指向它）——`Core.Carriers.Unit.MovementTickHandler.ResolveSpeed` 读值时若该属性完全未在 `stat.definition` 登记会直接抛 `ArgumentException`（不是"缺省退回 `MovementOptions.DefaultSpeed`"那种缺值场景，是"属性本身未定义"），玩家一旦提交任何移动意图就会装配期之后的第一次 `FixedUpdate` 崩溃；只要给这个 id 登记一行（`default_base` 可以就是 `MovementOptions.DefaultSpeed` 的量级，模板给了 5），单位没有显式设置过时会自动落回这个缺省基值 |
...
| `item.slot_definition` | 游戏必填（若已有 `item.template` 内容）/ 空壳（尚无物品内容时） | 数值设计落地阶段 N0（分阶段落地计划 T-N0-7、拍板 10）补的空壳表：`item/item.slot_definition.json` 只有空 `rows`，槽位清单、`is_weapon`/`is_equipment` 与阶段 N2（T-N2-1、ADR-0032 决策 1）补的 `budget_coefficient`/`price_coefficient`（槽位预算/价格系数）全部由具体游戏填写，模板里不写任何具体系数 |
| `item.quality_definition` | 游戏必填（若已有 `item.template` 内容）/ 空壳（尚无物品内容时） | 同上：`item/item.quality_definition.json` 只有空 `rows`，品质分档与 `budget_multiplier`（预算倍率）、阶段 N2（T-N2-1、ADR-0032 决策 2）补的 `affix_count`（可选）/`grant_budget_share`/`price_multiplier` 由具体游戏填写，模板里不写任何具体系数——`budget_multiplier`/`price_multiplier` 大小顺序须与 `sort_weight` 一致（`ItemQualityMultiplierOrderRule`），空壳阶段无行不触发该校验 |
| `item.armor_curve` | 游戏必填（若已有护甲位物品内容）/ 空壳（尚无护甲消费时） | 分阶段落地计划 T-N2-1（ADR-0032 决策 4）新增的空壳表：`item/item.armor_curve.json` 只有空 `rows`，物品等级到护甲值的曲线由具体游戏填写；消费实现（护甲值写入 `StatHost`）随 T-N2-4 落地，本框架当前只登记数据形状 |
| `item.weapon_dps_curve` | 游戏必填（若已有武器位物品内容）/ 空壳（尚无武器消费时） | 分阶段落地计划 T-N2-1（ADR-0032 决策 4）新增的空壳表：`item/item.weapon_dps_curve.json` 只有空 `rows`，物品等级到武器秒伤的曲线由具体游戏填写；消费实现（替换 `EquipmentHost.GetWeaponBaseDamage` 当前 `(min+max)/2`）随 T-N2-4 落地，本框架当前只登记数据形状 |
| `item.req_level_curve` | 游戏必填（若需按物品等级反推需求等级）/ 空壳（`requirements.level` 全部手填时） | 分阶段落地计划 T-N2-1（ADR-0032 决策 5）新增的空壳表：`item/item.req_level_curve.json` 只有空 `rows`，物品等级到需求等级的曲线由具体游戏填写；消费实现（`requirements.level` 缺省分支反推）随 T-N2-9 落地，本框架当前只登记数据形状 |
